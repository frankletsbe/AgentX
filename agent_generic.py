...
from agent_manager import initialize_huggingface, initialize_abacus, create_agent

# Use project-relative path for config.yaml
...
def main() -> None:
    ...
    try:
        ...
        # The 'config' instance already merges these, but we'll follow the script's logic
        # of showing where values come from.
        provider = yaml_provider if yaml_provider else config.get("INFERENCE_PROVIDER")
        model_name = yaml_model if yaml_model else config.get("MODEL_NAME")
        search_provider = yaml_search if yaml_search else config.get("SEARCH_PROVIDER")
        # Get enabled tools
...
# Debug output
        ...
        if provider.lower() == "huggingface":
            initialize_huggingface(config)
        elif provider.lower() == "abacus":
            initialize_abacus(config)
        else:
